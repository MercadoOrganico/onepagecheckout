...
<script language="JavaScript">
	...
	*/
    ...
    $(function(){
		$("input[name='wk_delivery_address_zip']").on('input', function (e) {
...
<script>
	function retiraHifenPostcodeDelivery() {
		var aux1 = document.getElementById("wk_delivery_address_zip").value;
		var aux2 = aux1.replace('-','');
		document.getElementById("wk_delivery_address_zip").value = aux2;
	}
</script>
...
<div id="wk-delivery-address-form">
	<div class="wk-delivery-address-form">
		<form method="POST" action="#" id="wk-delivery-form">
			...
			{if Configuration::get('WK_CHECKOUT_DELIVERY_DNI_SHOW')}
			<div class="form-group">
				<label class="label-control {if Configuration::get('WK_CHECKOUT_DELIVERY_DNI_REQ')}required{/if}">{l s='DNI Number' mod='wkonepagecheckout'}
				</label>
				<input
					{if Configuration::get('WK_CHECKOUT_DELIVERY_DNI_REQ')}data-required="1"{else}data-required="0"{/if}
					data-validate="isDniLite"
					type="text"
					name="wk_delivery_dni_info"
					id="wk_delivery_dni_info"
					onblur="validar_dni_delivery()"
					...
					class="form-control wkvalidatefield">
				<i class="material-icons wk-check-icon wkhide icon_wk_delivery_dni_info">&#xE876;</i>
				<i class="material-icons wk-error-icon wkhide error_wk_delivery_dni_info">&#xE001;</i>
				<span class="help-block wk-error wk_delivery_dni_info"></span>
			</div>
			{/if}
			...
			<div class="form-group">
				...
				<input
					...
					data-validate="isGenericName"
					type="text"
					name="wk_delivery_address_number"
					id="wk_delivery_address_number"
					value="{if isset($delivery_address)}{$delivery_address->number}{/if}"
					class="form-control wkvalidatefield">
				<i class="material-icons wk-check-icon wkhide icon_wk_delivery_address_number">&#xE876;</i>
				<i class="material-icons wk-error-icon wkhide error_wk_delivery_address_number">&#xE001;</i>
				<span class="help-block wk-error wk_delivery_address_number"></span>
			</div>
			{/if}
			...
			*}
			{if Configuration::get('WK_CHECKOUT_DELIVERY_ADDRESS_COMPLEM_SHOW')}
            {* Adriana - 15/07/2020 - fim *}	
			<div class="form-group">
			    {* Adriana - 15/07/2020 - início *}
			    {*
				...
				*}
				<label class="label-control {if Configuration::get('WK_CHECKOUT_DELIVERY_ADDRESS_COMPLEM_REQ')}required{/if}">{l s='Address Complement' mod='wkonepagecheckout'}
				</label>
			    {* Adriana - 15/07/2020 - fim *}
				<input
					maxlength="128"
					...
					*}
					{if Configuration::get('WK_CHECKOUT_DELIVERY_ADDRESS_COMPLEM_REQ')}data-required="1"{else}data-required="0"{/if}
			        {* Adriana - 15/07/2020 - fim *}
					data-validate="isAddress"
					type="text"
			...
			<div id="wk_delivery_address_state">
				{block name='wk_delivery_address_state'}
					<label class="label-control required">{l s='State' mod='wkonepagecheckout'}</label>
					<select name="wk_delivery_address_state" class="form-control wk_address_state" data-attr="delivery" id="id_state">
				...
				{/block}
				<span class="help-block wk-error wk_delivery_address_state"></span>
			</div>
			...
			<div class="form-group" style="text-align: right;">
				...
				<button type="submit" data-type="delivery" class="btn btn-primary wk-save-address" onclick="retiraHifenPostcodeDelivery()">
					{l s='Save' mod='wkonepagecheckout'}
				{* Adriana - 23/06/2020 - fim *}
				</button>
...
<script type="text/javascript">
	$("#wk_delivery_address_zip").focusout(function(){ 
		...
		$.ajax({
			...
			success: function(resposta){
				...
				$("#wk_delivery_address_info").val(resposta.logradouro);
				$("#wk_delivery_address_complement").val(resposta.bairro);
				$("#wk_delivery_address_city").val(resposta.localidade);
				// Adriana - 23/06/2020 - fim
				...
				$("#wk_delivery_address_number").focus();
			}
		});
		{* Adriana - 31/05/2021 - início *}
		}
...
<script>
	if (document.getElementById("wk_delivery_dni_info")){
		document.getElementById("wk_delivery_dni_info").addEventListener("keydown", mascaraDni);
		function mascaraDni() {
		if(event.keyCode != 46 && event.keyCode != 8 && event.keyCode != 9){
			var i = document.getElementById("wk_delivery_dni_info").value.length; //aqui pega o tamanho do input
			if (i === 3 || i === 7) //aqui faz a divisoes colocando um ponto no terceiro e setimo indice
			document.getElementById("wk_delivery_dni_info").value = document.getElementById("wk_delivery_dni_info").value + ".";
			else if (i === 11) //aqui faz a divisao colocando o hífen no decimo primeiro indice
			document.getElementById("wk_delivery_dni_info").value = document.getElementById("wk_delivery_dni_info").value + "-";
			else if (i === 14) 
				var dni_aux = document.getElementById("wk_delivery_dni_info").value;
				if (i === 14 && dni_aux.substr(4, 1) != '.'){
					document.getElementById("wk_delivery_dni_info").value = document.getElementById("wk_delivery_dni_info").value.replace(/[^0-9]/g, '');
					var str = document.getElementById("wk_delivery_dni_info").value;
					...
					document.getElementById("wk_delivery_dni_info").value = res;	
				}
...
<script>
	if (document.getElementById("wk_delivery_address_zip")){
		document.getElementById("wk_delivery_address_zip").addEventListener("keydown", mascaraPostcode);
		function mascaraPostcode() {
		if(event.keyCode != 46 && event.keyCode != 8 && event.keyCode != 9){
			var i = document.getElementById("wk_delivery_address_zip").value.length; //aqui pega o tamanho do input
			if (i === 5) //aqui faz a divisao colocando o hífen no quinto indice
				document.getElementById("wk_delivery_address_zip").value = document.getElementById("wk_delivery_address_zip").value + "-";
			}
		}
	}
</script>
...
<script>
{literal}
function validar_dni_delivery()
{
	dni = $('#wk_delivery_dni_info');
	
	dni.closest(".form-group").find('i').remove();
	dni.closest(".form-group").removeClass("has-error");

    //valida
	var dni_aux = $('#wk_delivery_dni_info').val();
	var dni_valido = validarCpfCnpjDelivery(dni_aux);
	if (!dni_valido) {
        $('#wk_delivery_dni_info').focus();
...
function validaCNPJDelivery(CNPJ) 
{
	var a = new Array();
	var b = new Number;
...
function validarCpfCnpjDelivery(valor) 
{
	...
	if (tam==11 ){
		if (!validaCPFDelivery(s)){
			return false;
		}
		return true;
	}		
	if (tam==14){
		if(!validaCNPJDelivery(s)){
			return false;			
		}
		return true;
	}
